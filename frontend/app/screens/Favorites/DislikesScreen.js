...
export default function MyDislikesScreen() {
  ...
  const fetchDislikes = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/likes/my-dislikes`, {
  ...
  const openMenu = (movie) => { setSelectedMovie(movie); setMenuVisible(true); };
  const closeMenu = () => { setSelectedMovie(null); setMenuVisible(false); };
  const goToMovie = () => {
  ...
  const removeDislike = async () => {
    ...
    try {
      ...
      await axios.post(
        `${API_BASE_URL}/likes`,
        { movieId: selectedMovie.tmdb_id, type: "dislike" },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchDislikes();
    } catch (error) {
    ...
    return (
      <View style={styles.itemContainer}>
        ...
        <View style={styles.infoContainer}>
          <TouchableOpacity onPress={() => navigation.navigate('MovieDetails', { movieId: movie.tmdb_id })}>
            <Text style={styles.title} numberOfLines={1}>{movie.title}</Text>
            <Text style={styles.subtitle}>Tu deste Dislike</Text>
          </TouchableOpacity>
        </View>
    ...
    >
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.headerTitle}>Meus Dislikes</Text>
        </View>
        ...
        >
          <Pressable style={styles.menuOverlay} onPress={closeMenu}>
            <View style={styles.menuContainer}>
              ...
              <TouchableOpacity style={styles.menuOption} onPress={removeDislike}>
                <Text style={[styles.menuOptionText, styles.menuOptionDanger]}>Remover Dislike</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  menuOptionDanger: { color: 'rgba(255,59,48,0.9)' },
});
